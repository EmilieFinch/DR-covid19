...
cm_force_rebuild_  = cm_option_("cm_force_rebuild", T)
cm_build_dir_      = cm_option_("cm_build_dir", here(cm_path_, "/build/"))
cm_build_verbose_  = cm_option_("cm_build_verbose", T)
cm_force_shared_   = cm_option_("cm_force_shared", F)

# Attach code
source(here(cm_path_, "/R/shared/cmS_misc.R"))
source(here(cm_path_, "/R/shared/cmS_plot.R"))

if (cm_version_ == 1) {
    source(here(cm_path_, "/R/v1/cm1_backend.R"))
    source(here(cm_path_, "/R/v1/cm1_run.R"))
    source(here(cm_path_, "/R/v1/cm1_params.R"))
    source(here(cm_path_, "/R/v1/cm1_interventions.R"))
    source(here(cm_path_, "/R/v1/cm1_fit.R"))
} else if (cm_version_ == 2) {
    source(here(cm_path_, "/R/v2/cm2_backend.R"))
    source(here(cm_path_, "/R/v2/cm2_funcs.R"))
    source(here(cm_path_, "/R/v2/cm2_run.R"))
    source(here(cm_path_, "/R/v2/cm2_params.R"))
    source(here(cm_path_, "/R/v2/cm2_interventions.R"))
    source(here(cm_path_, "/R/v2/cm2_fit.R"))
} else {
...
cm_matrices     = readRDS(here(cm_path_, "/data/all_matrices.rds"));
cm_populations  = readRDS(here(cm_path_, "/data/wpp2019_pop2020.rds"));
cm_structure_UK = readRDS(here(cm_path_, "/data/structure_UK.rds"));
cm_highrisk     = readRDS(here(cm_path_, "/data/prevalence_morbidities.rds"))
