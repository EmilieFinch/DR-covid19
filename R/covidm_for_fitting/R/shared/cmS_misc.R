...
# construct a delay distribution following an erlang distribution with mean mu and shape parameter shape.
cm_delay_erlang = function(mu, shape)
{
  if (shape != round(shape) | shape <= 0) {
    stop("Shape must be a positive integer.")
  }
  rate = shape / mu;
  return (data.table(p = rep(-rate, shape)))
}
