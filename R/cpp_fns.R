# Cpp functions for fitting 

# Function to give names to elements of x from priors
...
# Function to initialise list with values from x
cpp_vec <- function(x) {
...
# Function to wrap Cpp funcs for multi region fitting
...
# Function to name schedules
named_schedules <- function() {
  "enum { CH_CONTACT };"
}

# Function to create cpp changes
cpp_chgI_voc = function(priors, constants, v2, v2_relu, mobility_type)
{
  glue::glue(
    ...
    'for (unsigned int k : vector<unsigned int> { CH_CONTACT }) {',
    ...
    '        double othx = min((rtrc * 0.345 + trns * 0.445 + groc * 0.210), 1.24);', # If value is higher than 1.24 this gives a 0
    '        double t = P.changes.ch[k].times[i];',
    
    # Create vectors for baseline google mobility
    '         double bwplc = P.changes.ch[k].values[0][1];',
    '         double bothx = P.changes.ch[k].values[0][2];',
    
    if(mobility_type == "comix-base"){
      
      # Contacts between CoMix adjusted mobility and baseline google mobility
      '         double work = min((x_contact_adj_a * interp(wplc, work_curve)) + ((1 - x_contact_adj_a) * bwplc), 1.0);
                double home = P.changes.ch[k].values[i][0];
                double othe = min((x_contact_adj_a * interp(othx, other_curve)) + ((1 - x_contact_adj_a) * bothx), 1.0);'
    ...
    '    }',
    '    for (unsigned int i = 366; i < P.changes.ch[k].times.size(); ++i) {',
    '        //double resi = P.changes.ch[k].values[i][0];',
    '        double wplc = P.changes.ch[k].values[i][1];',
    '        double groc = P.changes.ch[k].values[i][2];',
    '        double rtrc = P.changes.ch[k].values[i][3];',
    ...
    if(mobility_type == "comix-base"){
      ...
      '         double work = min((x_contact_adj_b * interp(wplc, work_curve)) + ((1 - x_contact_adj_b) * bwplc), 1.0);
                double home = P.changes.ch[k].values[i][0];
                double othe = min((x_contact_adj_b * interp(othx, other_curve)) + ((1 - x_contact_adj_b) * bothx), 1.0);'
...
# Function to create c++ likelihood components
cpp_likI_voc = function(params, ld, sitreps, virus, sgtfd, sero, popid, max_date, priors, constants, death_cutoff, use_sgtf) {
...
# Function to create c++ observer function
cpp_obsI_voc = function(v2, P.death, P.critical, P.hosp, priors, constants, voct, voci) {
  glue::glue(
    named_params(priors,constants),
    named_schedules(),
    'auto asc = [&](double x, double y0, double y1, double s0, double s1) {',
    ...
    # Calculate logistic growth curve for increase in transmission
    'vector <int> voct = ${cpp_vec(voct)};',
    'vector <double> voci = ${cpp_vec(voci)};',
    
    ' int length_time = voct.size();',
    
    'vector <double> vocf(length_time);',
    'for (unsigned int l = 0; l < length_time; l++){',
    ' double seq_val = voci[l];',
    ' vocf[l] = asc(seq_val, 1, x_extra_voc_relu, -5, 5);',
    '}',
    
    'vector <double> vocv = vocf;',
    
    'for (unsigned int j = 0; j < (vocf.size()-1); j++){',
    'int r = vocf.size() - j - 1;',
    ' vocv[r] = vocf[r] / vocf[r-1];',
    '}',
    
    ' for (unsigned int i = 0; i < voct.size(); i++){',
    '   int thist = voct[i];',
    ' if (t == thist) {',
    
    'for (unsigned int q = 0; q < P.pop[0].u.size(); q++){',
    'P.pop[0].u[q] *= vocv[i];',
    ...
    '    double ihr_table[] = ${ cpp_vec(P.hosp) };',
    ...
    '        P.processes[11].prob[g][1] = 1 - P.processes[11].prob[g][0];',
    '        P.processes[0].prob[g][0]  = odds(ihr, (1 - adj_c) * x_hosp_rlo + adj_c * x_hosp_rlo2);',
    '        P.processes[0].prob[g][1]  = 1 - P.processes[0].prob[g][0];',
    '        P.processes[10].prob[g][0] = odds(ihr, (1 - adj_c) * x_hosp_rlo + adj_c * x_hosp_rlo2${ if (v2) "+ x_v2_hosp_rlo" else "" });',
    '        P.processes[10].prob[g][1] = 1 - P.processes[10].prob[g][0];',
    '    }',
    '}',
...
# Observer function for vaccination programmes
...
# Observer function for forced seasonality
